...
export interface ServiceSamplingStats {
  serviceName: string;
  sampledCount: number;
  totalCount: number;
...
export class AdaptiveSamplingStrategy implements SamplingStrategy {
  ...
  private processed = 0;
  private sampled = 0;
  private serviceStats = new Map<string, ServiceSamplingStats>();
  private errorThreshold = 0.1; // 10% error rate triggers higher sampling
  private volumeThreshold = 100; // High volume triggers sampling
  ...
  shouldSample(entry: LogEntry, context: SamplingContext): boolean {
    ...
    // Sample info/debug based on volume and recency
      ...
      else if (timeSinceLastSample > 30000) { // 30 seconds
...
export class PriorityBasedSamplingStrategy implements SamplingStrategy {
  ...
  private sampled = 0;
  private serviceStats = new Map<string, ServiceSamplingStats>();
  private priorityLevels = new Map<string, number>();
  ...
  private calculateBaseRate(priority: number): number {
    ...
    if (priority >= 100) return 1.0; // Always sample errors
    if (priority >= 80) return 0.8;  // 80% for warnings
    if (priority >= 60) return 0.5;  // 50% for HTTP logs
    if (priority >= 40) return 0.2;  // 20% for info logs
...
export class BurstSamplingStrategy implements SamplingStrategy {
  ...
  private sampled = 0;
  private serviceStats = new Map<string, ServiceSamplingStats>();
  private burstWindows = new Map<string, { count: number; startTime: number }>();
  private burstThreshold = 50; // Messages per second considered a burst
  private burstSamplingRate = 0.1; // 10% sampling during bursts
  private normalSamplingRate = 0.5; // 50% sampling during normal operation
  ...
  shouldSample(entry: LogEntry, context: SamplingContext): boolean {
    ...
    if (!burstWindow || (now - burstWindow.startTime) > 1000) { // New 1-second window
